...
#PROMPT='$FG[238]$(put_spacing)%{$reset_color%}
#%{$fg_bold[magenta]%}%n%{$reset_color%} at %{$fg_bold[yellow]%}%m%{$reset_color%} %{$fg_bold[green]%}$(collapse_pwd)%{$reset_color%}$(hg_prompt_info)$(git_prompt_info)$(git_prompt_status)
#$(virtualenv_info)%{$fg_bold[green]%}$(prompt_char)%{$reset_color%} '
...
#ZSH_THEME_GIT_PROMPT_PREFIX=" %{$fg_bold[yellow]%} ⭠ %{$reset_color%}$nhan_orange"
